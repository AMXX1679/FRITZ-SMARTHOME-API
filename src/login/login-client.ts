...
export class LoginClient {
    ...
    private async login(): Promise<SessionResponse> {
        ...
        const challengeResponse = await signChallenge(this.config.password, challenge);
    ...
    private async getLoginChallenge(): Promise<LoginChallenge> {
        ...
        if (typeof challenge !== 'string') {
            throw new Error('Expected challenge to be string but got ' + JSON.stringify(challenge)
                + ' ' + JSON.stringify(sessionResponse));
        }
        return parseChallenge(challenge);
    }

    private async loginWithChallengeResponse(challengeResponse: string): Promise<SessionResponse> {
        console.debug('Request login session', {
            username: this.config.username,
            response: challengeResponse
        });
        const sessionResponse = await axios.post(this.loginUrl, {
            username: this.config.username,
            response: challengeResponse
        }, {
    ...
    private async validateSession(): Promise<void> {
        ...
        if (sessionResponse.SessionInfo.SID._text === this.session.SessionInfo.SID._text) {
